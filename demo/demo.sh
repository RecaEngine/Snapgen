...
pei "docker buildx create --name snapgen-builder --use --buildkitd-flags '--allow-insecure-entitlement security.insecure'"
...
pei "docker buildx build --allow security.insecure --file 'snapgen-finetune.yaml' --output '_output' --target unsloth --progress plain ."
...
cat > snapgen-inference.yaml << EOF
#syntax=ghcr.io/sozercan/snapgen:latest
debug: true
apiVersion: v1alpha1
runtime: cuda
...
config: |
  - name: mistral-finetuned
    parameters:
      model: snapgen-model-q4_k_m.gguf
    context_size: 4096
    gpu_layers: 35
    f16: true
    mmap: true
...
pei "bat snapgen-inference.yaml"
...
pei "docker buildx build -t mistral-finetuned -f snapgen-inference.yaml --load --progress plain _output"
...
echo "❤️ In this demo, we have shown how to fine tune a model and deploy it as a minimal container using Snapgen. Thank you for watching!"

echo ""

# pei "docker buildx rm snapgen-builder"
