...
import (
	...
	"github.com/sozercan/snapgen/pkg/version"
	"gopkg.in/yaml.v2"
)
...
func Snapgen2LLB(c *config.FineTuneConfig) llb.State {
	...
	if c.Target == utils.TargetUnsloth {
		...
		unslothScriptURL := fmt.Sprintf("https://raw.githubusercontent.com/sozercan/snapgen/%s/pkg/finetune/target_unsloth.py", version)
