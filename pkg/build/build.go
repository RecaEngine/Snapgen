...
import (
	...
	"github.com/sozercan/snapgen/pkg/snapgen/config"
	"github.com/sozercan/snapgen/pkg/snapgen2llb/finetune"
	"github.com/sozercan/snapgen/pkg/snapgen2llb/inference"
	"github.com/sozercan/snapgen/pkg/utils"
...
const (
	...
	localNameSnapgenfile   = "snapgenfile.yaml"
	defaultSnapgenfileName = "snapgenfile.yaml"
...
func Build(ctx context.Context, c client.Client) (*client.Result, error) {
	inferenceCfg, finetuneCfg, err := getSnapgenfileConfig(ctx, c)
	if err != nil {
		return nil, errors.Wrap(err, "getting snapgenfile")
	}
...
func buildFineTune(ctx context.Context, c client.Client, cfg *config.FineTuneConfig) (*client.Result, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "validating snapgenfile")
	}
...
// buildImage builds an image from the given snapgenfile config.
func buildImage(ctx context.Context, c client.Client, cfg *config.InferenceConfig, convertOpts *d2llb.ConvertOpt) (*buildResult, error) {
	...
	state, image, err := inference.Snapgen2LLB(cfg, convertOpts.TargetPlatform)
	if err != nil {
		return nil, err
	}
...
func getSnapgenfileConfig(ctx context.Context, c client.Client) (*config.InferenceConfig, *config.FineTuneConfig, error) {
	opts := c.BuildOpts().Opts
	filename := opts[keyFilename]
	if filename == "" {
		filename = defaultSnapgenfileName
	}

	name := "load snapgenfile"
	if filename != "snapgenfile.yaml" {
		name += " from " + filename
	}
	...
	default:
		localSt := llb.Local(localNameDockerfile,
			...
			llb.SharedKeyHint(defaultSnapgenfileName),
	...
	if err != nil {
		return nil, nil, errors.Wrapf(err, "failed to resolve snapgenfile")
	}
	...
	dtSnapgenfile, err := ref.ReadFile(ctx, client.ReadRequest{
		Filename: filename,
	})
	if err != nil {
		return nil, nil, errors.Wrapf(err, "failed to read snapgenfile")
	}

	inferenceCfg, finetuneCfg, err := config.NewFromBytes(dtSnapgenfile)
	if err != nil {
...
func validateInferenceConfig(c *config.InferenceConfig) error {
	...
	if (slices.Contains(c.Backends, utils.BackendExllamaV2) || slices.Contains(c.Backends, utils.BackendMamba) || slices.Contains(c.Backends, utils.BackendDiffusers)) && c.Runtime != utils.RuntimeNVIDIA {
		return errors.New("exllama, mamba, and diffusers backends only supports nvidia cuda runtime. please add 'runtime: cuda' to your snapgenfile.yaml")
	}
