...
push-distroless-base:
	docker buildx build . -t sozercan/snapgen-base:latest -f Dockerfile.base --platform linux/amd64,linux/arm64 \
		--sbom=true --push
...
release-manifest:
	@sed -i "s/appVersion: $(VERSION)/appVersion: ${NEWVERSION}/" ./charts/snapgen/Chart.yaml
	@sed -i "s/version: $$(echo ${VERSION} | cut -c2-)/version: $$(echo ${NEWVERSION} | cut -c2-)/" ./charts/snapgen/Chart.yaml
